...
class TepungTeriguNonCurahChart
{
    protected $chart;
    ...
    public function build(): \ArielMejiaDev\LarapexCharts\LineChart
    {
        ...
        $data_2023 = Komoditi::select(DB::raw("CAST(AVG(tepung_terigu_non_curah) as int) as rata_bulan"))
                        ->whereYear('tanggal',$tahun_kemarin)
                        ->GroupBy(DB::raw("Month(tanggal)"))
                        ->orderBy(DB::raw("Month(tanggal)"), 'ASC')
                        ->pluck('rata_bulan')->toArray();
        
        $data_2024 = Komoditi::select(DB::raw("CAST(AVG(tepung_terigu_non_curah) as int) as rata_bulan"))
        ...
        ->setTitle('Tepung Terigu Non Curah')
